...
* месеца има по-малко от 2 цифри, той се префиксва с нули(например `09`)
* деня от месеца има по-малко от 2 цифри, той се префиксва с нули(например `07`)

`LazyMode::Date` трябва да има 4 метода:

* `#year` - връща като число годината
* `#month` - връща като число месеца
* `#day` - връща като число деня от месеца
* `#to_s` - връща стринга от който сме подстроили датата

Когато искаме нещо да връща време от тук нататък ще работим само с този обект. За удобство ще приемем, че в годината има 12 месеца и 30 дни във всеки месец.
...
* `#header` - връща стринг - заглавие на бележката ни
* `#file_name` - връща стринг - име на файла, в който пазим бележката
* `#body` - връща стринг - текстово съдържание на нашата бележка
...
`#create_file` приема името на файла като аргумент и блок(***няма да се извиква без блок***). В блока трябва да имаме достъп до метода `#note`, който създава нова бележка. Резултатът от `#create_file` е `LazyMode::File`, който има дефинирани `#name` - името на файла, `#notes` - списък с бележките от тип `LazyMode::Note`.

### `#note`

Създава нова бележка с `#header`, подаден като първи аргумент и приема след това произволен брой(нула или повече) тагове. Редът на таговете се пази както е подаден. Самият метод приема блок в който трябва да имаме достъп до следните методи:

* `#status` - приема символ, което е едно от `:postponed`, `:topostpone`. Няма да се подават невалидни стойности. При липса на извикване на такъв, се приема, че стойността е `:topostpone`.
* `#body` - приема произволен текст, който отговаря на съдържанието на бележката. При липса на такъв се приема, че стойноста на `#body` е празен стринг.
* `#scheduled` - дата във формата който е описан в `LazyMode::Date`. Наличието му е задължително за създаване на бележки. Приемаме, че винаги имаме подаден такъв.
...
`#scheduled` може да създава бележка, която се повтаря периодично от посочената дата. Това става като към датата долепим повторител във формата `+{n}{m,d,w}`.
Където `n` е число и означава всеки n-пъти, `m` - всеки месец, `d` - всеки ден, `w` - всяка седмица. В примера по-горе това означава, че сме насрочили плуване от `2012-08-07` през всеки 2 седмици.

### Влагане на бележки

Можем да влагаме бележките една в друга. Пример:
...
Влагането става като дефинирането на нови бележки. За да дефинираме влагане просто извикваме отново `#note` в блока за бележката, за която искаме да направим подбележка.

## Agenda

`LazyMode::File` трябва да дефинира и метода `#daily_agenda`. `#daily_agenda` приема като аргумент дата `date` и ни връща обект който има дефиниран метод `#notes`. Метода `#notes` трябва да ни върне спъсък от всички бележки които са `schedule`-нати за `date`. Пример:
...
Всяка бележка от `notes` трябва да има всички методи на `LazyMode::Note` и допълнителен метод `#date` - дата, която ни показва датата, за която е несрочено събитието. В случая с нашето `daily_agenda`, това винаги ще е датата, подадена като аргумент на `#daily_agenda`.


Аналогично дефинираме и метода `#weekly_agenda(date)` за `LazyMode::File`. Трябва да върнем обект, който имплементира `#notes`, и връща всички бележки за седем дни от `date`. Пример:
...
Обектите връщани от `#daily_agenda` и `#weekly_agenda` трябва да имплементират и метода `#where`. `#where` позволява филтриране по `status`, `tag` и `text`. `#where` приема следните keyword аргументи - `:tag`, `:text` или `:status` и връща нов обект, поддържащ същите методи като обектите върнати от `#daily_agenda` и `#weekly_agenda`. Пример:
...
* `:tag` - приема произволен таг, по който се филтрира. Бележките, които се връщат, трябва да имат този таг. ***Забележка*** таговете не се наследяват, т.е. влагането няма значение. Само директните за бележката тагове се гледат, таговете на нейния родител - не.

* `:text` - приема регулярен израз и филтрира бележките спрямо това дали регулярният израз мачва `#header` или `#body` на бележката.
* `:status` - приема един от възможните статуси `:postponed`, `:topostpone` и филтрира бележките спрямо това дали `#status`-ът им съвпада с подадения.
