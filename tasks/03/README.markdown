...
- Конструктор - приема обекта, които файлът ще съхранява (един от горните типове). Ако не се подадат данни да се съхрани `nil`.
...
Съхранението се състои в това една директория, заедно с всички нейни поддиректории и файлове, да бъде "превърната" в низ. Този низ, в последствие, може да бъде записан във файл. След време този файл може да бъде зареден, и низа в него да бъде "превърнат" отново в същата йерархия от директории и файлове.
...
За да ви улесним (или объркаме), сме измислили формат, в който трябва да стане това. Тук може би трябва да споменем, че, в 'реалният живот'™, не е добра идея да си измисляте собствени формати за подобни неща.

### Сериализация на файлове

Метода `serialize` на класа `File` **трябва да връща string**, в следния формат:

	<data_type>:<data>

Тук `<data_type>` трябва да се замени с типът на информацията, която се съхранява във файла (`nil`, `string`, `symbol`, `number` или `boolean`). `<data>` е текстовото представяне на самата информация (това, което връща `to_s` върху обекта, който съхраняваме).
...
В конкретния случай имаме само една поддиректория. Името и е `rbfs`, а данните в нея са с дължина 4 символа.
...
Броят на файловете и поддиректориите е 0, затова и след двуеточията няма изредени такива.
...
Метода `serialize` на класа `Directory`, трябва да генерира стринг в гореописания формат.
...
Ако не си пускате примерните тестове е 99.9% сигурно, че нещо ще се обърка. Лесно е да пропуснете някое двуеточие, или да добавите излишно. Проверявайте си задачата с нашите тестове!
